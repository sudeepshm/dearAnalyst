...
export default function SheetColumnPickerModal({
  ...
  // Determine active dataset to display based on view orientation
  const activeDataset = useMemo(() => {
    if (rawSheet) {
      if (viewOrientation === 'transposed' && rawSheet.transposed) {
        return rawSheet.transposed;
      }
      if (viewOrientation === 'standard' && rawSheet.standard) {
  ...
  const displayColumns = activeDataset.columns || [];
  const displayTypes = activeDataset.columnTypes || {};
  const displayData = activeDataset.data || [];

  // Filter columns based on search
  const filteredColumns = useMemo(() => {
    if (!searchQuery.trim()) return displayColumns;
    const q = searchQuery.toLowerCase().trim();
    return displayColumns.filter((c) => c.toLowerCase().includes(q));
  ...
  const targetFieldTabs = useMemo(() => {
    if (chartType === 'candlestick') {
      return [
        { key: 'xField', label: 'Timeline (Date)', currentVal: fieldValues.xField, icon: Calendar, color: 'text-blue-400' },
        { key: 'openField', label: 'Open Price', currentVal: fieldValues.openField, icon: TrendingUp, color: 'text-emerald-400' },
        { key: 'closeField', label: 'Close Price', currentVal: fieldValues.closeField, icon: TrendingUp, color: 'text-rose-400' },
        { key: 'lowField', label: 'Lowest (Low)', currentVal: fieldValues.lowField, icon: TrendingUp, color: 'text-amber-400' },
        { key: 'highField', label: 'Highest (High)', currentVal: fieldValues.highField, icon: TrendingUp, color: 'text-cyan-400' },
      ];
    }

    if (chartType === 'scatter') {
      return [
        { key: 'xField', label: 'X-Axis (Independent)', currentVal: fieldValues.xField, icon: Hash, color: 'text-blue-400' },
        { key: 'yField', label: 'Y-Axis (Dependent)', currentVal: fieldValues.yField, icon: Hash, color: 'text-emerald-400' },
      ];
    }

    if (chartType === 'pie') {
      return [
        { key: 'xField', label: 'Timeline / Slices', currentVal: fieldValues.xField, icon: Calendar, color: 'text-blue-400' },
        { key: 'yField', label: 'Metric Value', currentVal: fieldValues.yField, icon: Layers, color: 'text-emerald-400' },
        { key: 'yFields', label: 'Components (Multi)', count: (fieldValues.yFields || []).length, icon: Layers, color: 'text-cyan-400', isMulti: true },
      ];
    }

    // Default for combo, bar, line, waterfall, etc.
    return [
      { key: 'xField', label: 'Timeline (X-Axis)', currentVal: fieldValues.xField, icon: Calendar, color: 'text-blue-400' },
      { key: 'yFields', label: 'Series Metrics (Y-Axis)', count: (fieldValues.yFields || []).length, icon: Layers, color: 'text-cyan-400', isMulti: true },
      { key: 'yField', label: 'Left Axis (Y1)', currentVal: fieldValues.yField, icon: Hash, color: 'text-blue-400' },
      { key: 'y2Field', label: 'Right Axis (Y2)', currentVal: fieldValues.y2Field, icon: Hash, color: 'text-purple-400' },
    ];
  }, [chartType, fieldValues]);

  const activeTabConfig = targetFieldTabs.find((t) => t.key === targetField) || targetFieldTabs[0];
  ...
  const handleColumnClick = (colName) => {
    ...
    const values = displayData.map((r) => r[colName]).filter((v) => v !== '' && v !== null && v !== undefined);
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-3 sm:p-6 bg-slate-950/80 backdrop-blur-md animate-in fade-in duration-200">
      <div 
        className="w-full max-w-6xl max-h-[92vh] flex flex-col bg-[#0b101e] border border-blue-500/30 rounded-2xl shadow-[0_0_50px_rgba(19,100,226,0.25)] overflow-hidden"
        ...
        <div className="flex-1 overflow-auto bg-[#070b14] relative scrollbar-thin max-h-[55vh]">
          <table className="w-full text-left border-collapse text-xs font-mono">
            ...
            <thead className="sticky top-0 z-20 bg-slate-900 shadow-md">
              ...
              <tr className="border-b border-slate-750 bg-slate-900/95">
                ...
                {filteredColumns.map((col) => {
                  ...
                  const isTargetAssigned = !isMultiSelectMode && fieldValues[targetField] === col;
                  const isMultiSelected = isMultiSelectMode && (fieldValues.yFields || []).includes(col);
                  const selectedSeriesIndex = (fieldValues.yFields || []).indexOf(col);

                  // Check if assigned to any other field
                  const otherAssignment = Object.entries(fieldValues).find(
            ...
            <tbody className="divide-y divide-slate-850/50">
              {displayData.slice(0, 35).map((row, rIdx) => (
