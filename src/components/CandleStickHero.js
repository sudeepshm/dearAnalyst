...
export default function CandleStickHero({ onStartStory }) {
  ...
  useEffect(() => {
    ...
    const draw = () => {
      ...
      // Clean architectural grid lines on pitch black
      ctx.strokeStyle = 'rgba(255, 255, 255, 0.08)';
      ctx.lineWidth = 1;
      // Horizontal price lines
      for (let y = 70; y < H - 50; y += 45) {
        ctx.beginPath();
        ctx.moveTo(0, y);
        ctx.lineTo(W, y);
        ctx.stroke();
      ...
      drawLine(ema21, 'rgba(168, 85, 247, 0.60)', 1.5, [4, 3]);
      drawLine(ema9,  'rgba(56, 189, 248, 0.85)', 2);
  ...
  return (
    <section className="relative w-full min-h-screen flex flex-col overflow-hidden">
      ...
      <header className="relative z-30 w-full px-6 sm:px-10 py-5 flex items-center justify-between">
        ...
        <div className="flex items-center gap-3">
          <div className="relative w-8 h-8 flex-shrink-0">
            ...
            <div className="absolute inset-[1px] rounded-[7px] bg-black flex items-center justify-center">
          ...
          <div>
            ...
            <div className="font-mono text-[9px] text-slate-400 uppercase tracking-widest leading-none mt-0.5">
        ...
        <div className="hidden sm:flex items-center gap-3 px-4 py-2 rounded-full glass-card border-[var(--border-subtle)] text-xs font-mono">
          ...
          <span className="text-white font-semibold tabular-nums">{tickerPrice.toLocaleString('en-US', { minimumFractionDigits: 2 })}</span>
          <span className={`px-2 py-0.5 rounded-md text-[10px] font-bold ${isUp ? 'bg-emerald-500/20 text-emerald-400 border border-emerald-500/40' : 'bg-rose-500/20 text-rose-400 border border-rose-500/40'}`}>
      ...
      <div className="relative z-20 flex-1 flex flex-col items-center justify-center px-6 sm:px-12 pt-12 pb-20 text-center">
        ...
        {/* Pill badge */}
        <div className="inline-flex items-center gap-2 px-4 py-1.5 mb-8 rounded-full glass-card border border-[var(--border-subtle)] text-xs font-mono text-white">
          <Sparkles className="w-3.5 h-3.5 text-cyan-400" />
          <span>Interactive Financial Data Storytelling</span>
          <span className="w-1 h-1 rounded-full bg-cyan-400 opacity-80" />
          <span className="text-slate-300">14 Chart Types</span>
        </div>
        ...
        <div className="flex flex-col sm:flex-row items-center gap-3 mb-14">
          ...
          <a
            ...
            className="inline-flex items-center gap-2 px-6 py-3.5 rounded-xl text-[0.9rem] font-medium text-white hover:text-white glass-card hover:border-[var(--border-accent)] transition-all duration-200 border border-white/10"
          >
